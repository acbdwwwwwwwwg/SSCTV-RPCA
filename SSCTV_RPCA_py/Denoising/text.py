...
新增点（按你的需求）：
5) 噪声水平默认依次跑：0.1、0.2、0.3、0.4
   - 通过 --noise_levels 可改
   - 通过 --noise_param 控制噪声水平作用在 gaussian 还是 sparse（默认 sparse）
6) 输出的图和表都标注噪声水平：
   - 目录：result/<run_id>/noise_<noise_param>_<noise_level>/...
   - CSV：新增 noise_param, noise_level, gaussian_level, sparse_level, stripe_prob, stripe_strength
   - 图片：标题包含 noise_param=noise_level，文件名也包含噪声信息
"""
...
def format_noise_dir(noise_param: str, noise_level: float) -> str:
    """
    把噪声信息格式化为目录名：noise_sparse_0.10
    """
    return f"noise_{noise_param.lower()}_{noise_level:.2f}"


def format_noise_tag(noise_param: str, noise_level: float) -> str:
    """
    用于图片标题/表格标注的 tag
    """
    return f"{noise_param.lower()}={noise_level:.2f}"


def parse_noise_levels(s: str) -> List[float]:
    """
    解析 --noise_levels 字符串：支持 "0.1,0.2,0.3,0.4" 或 "0.1 0.2 0.3 0.4"
    """
    if s is None:
        return []
    s = s.strip()
...
def save_results_csv(path: str,
    ...
    with open(path, "w", newline="", encoding="utf-8") as f:
        w = csv.writer(f)
        w.writerow([
            "noise_param", "noise_level",
            "gaussian_level", "sparse_level", "stripe_prob", "stripe_strength",
            "case_id", "method", "mPSNR", "mSSIM", "ERGAS", "time_sec"
        ])
        for case_id, res in all_case_results:
            for (name, mpsnr_, mssim_, ergas_, sec) in res:
                w.writerow([
                    noise_param, f"{noise_level:.6f}",
                    f"{gaussian_level:.6f}", f"{sparse_level:.6f}",
                    f"{stripe_prob:.6f}", f"{stripe_strength:.6f}",
                    case_id, name,
                    f"{mpsnr_:.6f}", f"{mssim_:.6f}", f"{ergas_:.6f}", f"{sec:.6f}"
                ])
...
def save_average_csv(path: str,
    ...
    with open(path, "w", newline="", encoding="utf-8") as f:
        ...
        w.writerow([
            "noise_param", "noise_level",
            "gaussian_level", "sparse_level", "stripe_prob", "stripe_strength",
            "method", "AVG_mPSNR", "AVG_mSSIM", "AVG_ERGAS", "AVG_time_sec", "N_cases"
        ])
...
def run_one_case(clean_data: np.ndarray,
    ...
    if save_images:
        ...
        # ✅ 标注噪声水平
        plt.suptitle(
            f"Denoising - {case_id} | noise: {noise_tag} | gaussian={gaussian_level:.2f}, sparse={sparse_level:.2f}, stripe_prob={stripe_prob:.2f}",
            y=0.98,
            fontsize=12
        )
        plt.tight_layout()
        ...
        # ✅ 图片文件名也带噪声信息，避免覆盖
        out_png = os.path.join(img_dir, f"overview_{noise_param.lower()}_{noise_level:.2f}.png")
...
def _worker_run_one(payload: Dict):
    ...
    res = run_one_case(
        ...
        noise_param=payload["noise_param"],
...
# =========================
# CLI：默认不传参也会跑ALL+全核+噪声levels(0.1/0.2/0.3/0.4)
# =========================
def parse_args():
    ...
    p = argparse.ArgumentParser(description="HSI/MSI Denoising Demo (default: ALL datasets + ALL cases + all CPU cores + noise levels 0.1/0.2/0.3/0.4)")
    ...
    # 单次噪声参数（仍保留），但默认会被 noise_levels + noise_param 覆盖
    p.add_argument("--gaussian", type=float, default=0.0, help="高斯噪声强度（std 系数）")
    p.add_argument("--sparse", type=float, default=0.3, help="稀疏噪声比例（脉冲点概率）")
    p.add_argument("--stripe_prob", type=float, default=0.0)
    p.add_argument("--stripe_strength", type=float, default=0.1)

    # ✅ 新增：噪声水平批量跑（默认 0.1/0.2/0.3/0.4）
    p.add_argument("--noise_levels", type=str, default="0.1,0.2,0.3,0.4",
                   help="要跑的噪声水平列表，如 '0.1,0.2,0.3,0.4' 或 '0.1 0.2 0.3 0.4'")
    p.add_argument("--noise_param", type=str, default="sparse", choices=["sparse", "gaussian"],
                   help="噪声水平作用在哪个参数：sparse(稀疏噪声比例) 或 gaussian(高斯强度)。默认 sparse")
...
def run_dataset_cases(dataset_name: str,
    ...
    noise_tag = format_noise_tag(noise_param, noise_level)
    ...
    # ✅ CSV 文件名也标注噪声水平
    per_case_csv = os.path.join(ds_dir, f"metrics_per_case_{noise_param.lower()}_{noise_level:.2f}.csv")
    save_results_csv(
        per_case_csv, all_case_results,
        noise_param=noise_param, noise_level=noise_level,
        gaussian_level=gaussian_level, sparse_level=sparse_level,
        stripe_prob=stripe_prob, stripe_strength=stripe_strength
    )

    agg = aggregate_all_results(all_case_results)
    avg_csv = os.path.join(ds_dir, f"metrics_average_{noise_param.lower()}_{noise_level:.2f}.csv")
    save_average_csv(
        avg_csv, agg,
        noise_param=noise_param, noise_level=noise_level,
        gaussian_level=gaussian_level, sparse_level=sparse_level,
...
def run():
    ...
    # 基础噪声参数（当 noise_param 不作用在它时，作为“另一个噪声分量”的固定值）
    base_gaussian = float(args.gaussian)
    base_sparse = float(args.sparse)
    stripe_prob = float(args.stripe_prob)
    stripe_strength = float(args.stripe_strength)
    seed = int(args.seed)

    noise_param = args.noise_param.lower().strip()
    noise_levels = parse_noise_levels(args.noise_levels)

    if not noise_levels:
        # 兜底：若用户传空，至少跑一次（用 base 参数）
        noise_levels = [base_sparse if noise_param == "sparse" else base_gaussian]
    ...
    with open(os.path.join(run_dir, "README.txt"), "w", encoding="utf-8") as f:
        ...
        f.write(f"base_noise: gaussian={base_gaussian}, sparse={base_sparse}, stripe_prob={stripe_prob}, stripe_strength={stripe_strength}\n")
    ...
    with open(summary_csv, "w", newline="", encoding="utf-8") as fsum:
        ...
        wsum.writerow([
            "noise_param", "noise_level",
            "gaussian_level", "sparse_level", "stripe_prob", "stripe_strength",
            "dataset", "num_cases", "per_case_csv", "avg_csv", "error_file"
        ])

        for nl in noise_levels:
            # 计算本轮实际 gaussian/sparse
            if noise_param == "sparse":
                gaussian_level = base_gaussian
                sparse_level = float(nl)
            else:
                gaussian_level = float(nl)
                sparse_level = base_sparse

            noise_subdir_name = format_noise_dir(noise_param, float(nl))
            noise_run_dir = ensure_dir(os.path.join(run_dir, noise_subdir_name))

            # 每个噪声水平写个 README
            with open(os.path.join(noise_run_dir, "README.txt"), "w", encoding="utf-8") as fn:
                fn.write("该目录为本噪声水平的运行输出。\n")
                fn.write(f"run_id={run_id}\n")
                fn.write(f"noise_param={noise_param}\n")
                fn.write(f"noise_level={float(nl):.6f}\n")
                fn.write(f"gaussian_level={gaussian_level:.6f}\n")
                fn.write(f"sparse_level={sparse_level:.6f}\n")
                fn.write(f"stripe_prob={stripe_prob:.6f}\n")
                fn.write(f"stripe_strength={stripe_strength:.6f}\n")
                fn.write(f"dataset={args.dataset}\n")
                fn.write(f"case={args.case}\n")
                fn.write(f"jobs={jobs}\n")
                fn.write(f"seed={seed}\n")
            ...
            for ds in datasets_to_run:
                try:
                    ...
                    wsum.writerow([
                        noise_param, f"{float(nl):.6f}",
                        f"{gaussian_level:.6f}", f"{sparse_level:.6f}",
                        f"{stripe_prob:.6f}", f"{stripe_strength:.6f}",
                        info.get("dataset", ""),
                        info.get("num_cases", 0),
                        info.get("per_case_csv", ""),
                        info.get("avg_csv", ""),
                        ""
                    ])
                except Exception as e:
                    err_path = os.path.join(noise_run_dir, f"{ds}_ERROR.txt")
    ...
    print(f"[Info] 噪声批量：noise_param={noise_param}, levels={noise_levels}")
    print(f"[Info] 每个噪声水平会在 run_dir 下生成子目录：noise_<param>_<level>/...")
